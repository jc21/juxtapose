...
const internalService = {
	...
	create: (access, data) => {
		...
		return access
			.can("services", data)
			.then(() => {
				return serviceModel.query().omit(omissions()).insertAndFetch(data);
			})
			.then((service) => {
				if (
					service.type === "slack" ||
	...
	update: (access, data) => {
		return access
			.can("services", data.id)
			.then(() => {
				if (data?.type?.match(/(.|\n)*-webhook$/im) || data.type === "gchat") {
					return internalService.generateEndpointToken(data).then((token) => {
	...
	get: (access, data) => {
		return access
			.can("services", data.id)
			.then(() => {
				const query = serviceModel.query().where("id", data.id).first();
	...
	getCount: (access, search_query) => {
		return access
			.can("services")
			.then(() => {
				const query = serviceModel.query().count("id as count").first("count");
	...
	test: (access, data) => {
		...
		return access.can("services", data.id).then(() => {
			return internalServiceWorker.sendMessage(
				data.id,
				data.username,
				data.message,
			);
	...
	getUsers: (access, service_id) => {
		...
		return access.can("services:users").then(() => {
	 ...
	 */
	getAvailable: (access) => {
		return access.can("services:available").then(() => {
			return serviceModel
				.query()
				.where("is_deleted", 0)
	 ...
	 */
	getActiveServices: () => {
		return serviceModel.query().where("is_deleted", 0);
	},
	...
	generateEndpointToken: (row) => {
		...
		payload.jti = crypto.randomBytes(12).toString("base64").substr(-8);
